...
@router.post(
    "/exclude",
    response_model=ExcludedPortResponse,
    status_code=status.HTTP_201_CREATED,
)
async def exclude_port(
    admin: AdminUser,
    db: DbSession,
    request: PortExcludeRequest,
) -> ExcludedPortResponse:
    """Exclude a port from scan results (admin only)."""
    network = await networks_service.get_network_by_id(db, request.network_id)
    if network is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    exclusion = await excluded_ports_service.create_exclusion(
        db=db,
        network_id=request.network_id,
        ip=request.ip,
        port=request.port,
    ...
    return ExcludedPortResponse.model_validate(exclusion)
