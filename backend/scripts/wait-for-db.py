...
def wait_for_db(
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # If database is None, we'll connect without specifying a database (tests server availability)
            connect_kwargs = {
                'host': host,
                'port': port,
                'user': user,
            ...
            if attempt < max_retries:
                delay = min(retry_delay * (1.5 ** (attempt - 1)), 30.0)  # Exponential backoff, max 30s
                print(f"Database not ready yet (attempt {attempt}/{max_retries}): {e}")
                print(f"Retrying in {delay:.1f} seconds...")
                time.sleep(delay)
            else:
                print(f"ERROR: Database failed to become ready after {max_retries} attempts", file=sys.stderr)
