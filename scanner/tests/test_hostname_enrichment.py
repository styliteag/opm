...
class TestEnrichHostResults:
    ...
    def test_hackertarget_fallback(
        ...
        # crt.sh should not be called since HackerTarget resolved it
    ...
    def test_crt_sh_fallback(
        ...
        """crt.sh should only be queried for IPs not resolved by ip-api or HackerTarget."""
        mock_ip_api.return_value = {"1.1.1.1": "one.one.one.one"}
        mock_ht.return_value = {}
        mock_crt_sh.return_value = {"10.0.0.1": "server.example.com"}

        hosts = [
            HostResult(ip="1.1.1.1", hostname=None, is_pingable=True, mac_address=None, mac_vendor=None),
            HostResult(ip="10.0.0.1", hostname=None, is_pingable=True, mac_address=None, mac_vendor=None),
        ]
        ...
        # crt.sh should only have been called with the IP not resolved by ip-api/ht
