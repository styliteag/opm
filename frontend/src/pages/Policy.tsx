...
const Policy = () => {
    ...
    const policyQuery = useQuery({
        ...
        queryFn: () => {
            const url = filterNetworkId === 'all'
                ? '/api/policy'
                : `/api/policy?network_id=${filterNetworkId}`
        ...
        <div className="space-y-8 max-w-[1600px] mx-auto p-4 md:p-8">
            <div className="rounded-[2.5rem] border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-10 shadow-2xl relative overflow-hidden group">
                ...
                <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                    <div>
                        <h1 className="text-4xl font-black text-slate-900 dark:text-white tracking-tight leading-none">Security Policy</h1>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        {isAdmin && (
                            <button
                                ...
                                className={`px-6 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all active:scale-95 shadow-xl ${showAddForm
                ...
                {showAddForm && isAdmin && (
                    <form onSubmit={handleSubmit} className="mt-10 space-y-6 animate-in slide-in-from-top-4 duration-500">
                        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Port / Range</label>
                                <input
                                    type="text"
                                    value={newRule.port}
                            ...
                            <div className="space-y-2">
                                ...
                                <select
                                    ...
                                    onChange={(e) => setNewRule({ ...newRule, rule_type: e.target.value as 'allow' | 'block' })}
                                    className="w-full bg-slate-50 dark:bg-slate-950 border-2 border-slate-100 dark:border-slate-800 rounded-2xl px-4 py-3 text-sm font-bold focus:border-cyan-500 outline-none transition-all shadow-inner"
                                >
                        ...
                        <div className="flex justify-end">
                            <button type="submit" disabled={createMutation.isPending} className="px-10 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-2xl text-xs font-black uppercase tracking-widest shadow-2xl hover:scale-105 transition-all">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden min-h-[400px]">
                ...
                <div className="divide-y divide-slate-50 dark:divide-slate-800/30">
                    {policyQuery.isLoading ? (
                        <div className="p-20 text-center text-xs font-bold text-slate-400 uppercase tracking-widest animate-pulse">Syncing Cryptographic Security Layer...</div>
                    ) : rules.length === 0 ? (
                        <div className="p-20 text-center">
                            <p className="text-sm font-bold text-slate-400 italic">No rules found for this scope.</p>
                        </div>
                    ) : (
                        rules.map((rule) => (
                            <div key={`${rule.network_id}-${rule.id}`} className={`px-10 py-4 transition-all ${editingRule?.id === rule.id && editingRule.network_id === rule.network_id ? 'bg-cyan-50 dark:bg-cyan-900/10' : 'hover:bg-slate-50/50 dark:hover:bg-slate-800/10'}`}>
                                <div className="flex items-center gap-4">
                                    {/* Scope */}
                                    <div className="flex-1 min-w-0">
                                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${rule.network_id === null
                                    ...
                                    <div className="flex-[1.5] min-w-0 font-mono text-sm font-bold text-slate-900 dark:text-white">
                                        {editingRule?.id === rule.id && editingRule.network_id === rule.network_id ? (
                                            ...
                                            <>
                                                ...
                                                <span className="text-cyan-600 dark:text-cyan-400">{rule.port === "*" ? "EVERY PORT" : rule.port}</span>
                                            </>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex-1 text-center">
                                        {editingRule?.id === rule.id && editingRule.network_id === rule.network_id ? (
                                            <select
                                                ...
                                                onChange={e => setEditFields({ ...editFields, rule_type: e.target.value as 'allow' | 'block' })}
                                            ...
                                            <span className={`px-2 py-1 rounded text-[10px] font-black uppercase tracking-tighter border ${rule.rule_type === 'allow'
                                    ...
                                    <div className="flex-[2] min-w-0">
                                        {editingRule?.id === rule.id && editingRule.network_id === rule.network_id ? (
                                            ...
                                            <p className="text-xs font-medium text-slate-500 dark:text-slate-400 truncate hover:whitespace-normal cursor-help" title={rule.description || ''}>
                                                {rule.description || '—'}
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    <div className="w-48 flex items-center justify-end gap-2">
                                        {editingRule?.id === rule.id && editingRule.network_id === rule.network_id ? (
                                            <>
                                                <button onClick={handleSaveEdit} className="text-[10px] font-black uppercase text-cyan-600 hover:underline">Save</button>
                                                <button onClick={() => setEditingRule(null)} className="text-[10px] font-black uppercase text-slate-400 hover:underline">Cancel</button>
                                            </>
                                        ) : isAdmin && (
                                            <>
                                                <button onClick={() => handleStartEdit(rule)} className="p-2 text-slate-400 hover:text-cyan-500 transition-colors">
                                                    <svg className="w-4 h-4" fill="none" viewBox="0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                                </button>
                                                <button onClick={() => deleteMutation.mutate(rule)} disabled={deleteMutation.isPending} className="p-2 text-slate-400 hover:text-rose-500 transition-colors">
