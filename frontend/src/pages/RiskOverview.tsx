...
const RiskOverview = () => {
  ...
  const [sortColumn, setSortColumn] = useState<'severity' | 'ip' | 'port' | 'network' | 'time'>(
    'time',
  )
  ...
  const assignAlertMutation = useMutation({
    ...
    onError: (e) => {
      setToast({
        message: e instanceof Error ? e.message : 'Failed to update assignment',
        tone: 'error',
      })
      setUpdatingAssignment(null)
    },
  ...
  const handleAcknowledgeOnly = () => {
    ...
    if (actionModal.mode === 'single') {
      ...
      const hasSSH = alert.related_ssh_alert_count > 0
      singleAcknowledgeMutation.mutate({
        alertId: alert.id,
        reason,
        include_ssh_findings: hasSSH && includeSSH,
      })
    } else {
      const alertIds = actionModal.alerts.map((a) => a.id)
                ...
                {alertsQuery.isLoading || policyQuery.isLoading ? (
                  ...
                  filteredAlerts.map((alert) => {
                    ...
                    const portData = alert.global_open_port_id
                      ? portMap.get(alert.global_open_port_id)
                      : null
                              ...
                              <button
                                onClick={() => {
                                  setAssignDropdownOpen(
                                    assignDropdownOpen === alert.id ? null : alert.id,
                                  )
                                }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-md rounded-2xl border border-slate-200/70 bg-white p-6 shadow-2xl dark:border-slate-800/70 dark:bg-slate-900">
            <div className="mb-4 flex items-start justify-between">
              <div>
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white">
                  Acknowledge{' '}
                  {actionModal.mode === 'bulk' ? `${actionModal.alerts.length} Alerts` : 'Alert'}
                </h3>
            ...
            <p className="mb-4 text-sm text-slate-600 dark:text-slate-300">
              Why is {actionModal.mode === 'bulk' ? 'this' : 'this port'} okay? You can also add an
              acceptance rule so future scans won't alert again.
            </p>
            ...
            {actionModal.mode === 'single' && actionModal.alerts[0].related_ssh_alert_count > 0 && (
              <label className="mb-4 flex items-center gap-3 rounded-xl border border-amber-200 bg-amber-50/50 px-4 py-3 cursor-pointer transition hover:bg-amber-50 dark:border-amber-500/30 dark:bg-amber-500/5 dark:hover:bg-amber-500/10">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-amber-700 dark:text-amber-200">
                    Also acknowledge {actionModal.alerts[0].related_ssh_alert_count} SSH security{' '}
                    {actionModal.alerts[0].related_ssh_alert_count === 1 ? 'finding' : 'findings'}
                  </p>
                  <p className="text-xs text-amber-600/80 dark:text-amber-300/60">
                    Marks related SSH alerts (weak ciphers, auth issues, etc.) as acknowledged
                  </p>
                </div>
              </label>
            )}
