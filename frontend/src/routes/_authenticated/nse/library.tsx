...
import { Search, ArrowLeft, FileCode, Upload, Copy, Trash2, RefreshCw, RotateCcw } from 'lucide-react'
...
function NseLibraryPage() {
  ...
  const handleDelete = (e: React.MouseEvent, scriptName: string) => {
    e.preventDefault()
    e.stopPropagation()
    if (!confirm(`Delete custom script "${scriptName}"? It will be automatically removed from any profiles that use it.`)) return
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link to="/nse/profiles" className="rounded-md p-1 text-muted-foreground hover:text-foreground transition-colors">
      ...
      {filtered.length === 0 ? (
        <EmptyState title="No scripts found" message="Try a different search or category." icon={FileCode} />
      ) : (
          ...
          {filtered.map((script) => (
            ...
            >
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <p className="font-mono text-sm font-medium text-primary">{script.name}</p>
                ...
                <div className="flex items-center gap-1 ml-4">
                  <div className="flex flex-wrap gap-1 mr-2">
                    {script.categories?.slice(0, 3).map((cat) => (
                      <span key={cat} className="rounded bg-white/5 px-1.5 py-0.5 text-[10px] text-muted-foreground">
                        {cat}
                      </span>
                    ))}
                  </div>
