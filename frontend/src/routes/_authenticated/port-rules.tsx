...
import { Plus, Trash2, Globe, Network, ChevronDown, Power, PowerOff } from 'lucide-react'
...
function RuleRow({
  ...
  return (
    <div className={`flex items-center gap-3 px-5 py-3 group ${!rule.enabled ? 'opacity-50' : ''}`}>
      <Checkbox
        checked={selected}
      ...
      <div className="flex items-center gap-3 flex-1 min-w-0">
        ...
        />
        {!rule.enabled && (
          <StatusBadge label="disabled" variant="neutral" />
        )}
        {rule.description && (
...
function PortRulesPage() {
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: ['policy', 'rules'] })
  ...
  const isBulkPending = bulkDelete.isPending || bulkEnable.isPending || bulkDisable.isPending
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start">
              ...
              <DropdownMenuItem
                ...
                disabled={isBulkPending}
                onClick={() => {
                  if (confirm(`Delete ${selectedIds.size} rule(s) permanently?`)) {
