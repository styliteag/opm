...
function App() {
  ...
  const navItems = [
    ...
    { label: 'Alert Rules', to: '/port-rules' },
          ...
          {user ? (
            <div className="flex flex-wrap items-center gap-3 text-sm">
              ...
              <button
                type="button"
                onClick={() => void logout()}
                className="rounded-full border border-slate-200 px-4 py-2 text-xs font-semibold tracking-wide text-slate-700 transition hover:border-slate-300 hover:bg-slate-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/70 dark:border-slate-800 dark:text-slate-200 dark:hover:border-slate-700 dark:hover:bg-slate-900"
              >
                Logout
              </button>
            </div>
