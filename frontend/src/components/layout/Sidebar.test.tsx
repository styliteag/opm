...
vi.mock('@tanstack/react-router', () => ({
  Link: ({ children, to, ...props }: { children: React.ReactNode; to: string; className?: string; title?: string }) => (
...
describe('Sidebar', () => {
  ...
  it('renders the app name', () => {
    render(<Sidebar />)
    expect(screen.getByText('Sentinel Lens')).toBeInTheDocument()
  })
  ...
  it('hides admin items for non-admin users', () => {
    useAuthStore.setState({
      user: { id: 2, email: 'viewer@test.com', role: 'viewer', theme_preference: 'dark' },
      token: 'token',
      isAuthenticated: true,
    })
    render(<Sidebar />)
  ...
  it('hides labels when collapsed', () => {
    ...
    render(<Sidebar />)
    expect(screen.queryByText('Sentinel Lens')).not.toBeInTheDocument()
