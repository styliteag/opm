...
- **Hostname cache: manual edit + host-detail panel**: operators can now hand-curate cached vhost lists from the admin UI and see them on the host detail page. Three new endpoints + two UI surfaces: (1) `PUT /api/admin/hostname-lookup/entries/{ip}` (admin, body `CacheEntryUpdateRequest { hostnames: list[str] }`) replaces the row for an IP with a manual edit — service layer stamps `source='manual'`, derives `status='success'` or `'no_results'` from whether the list is empty, and sets `expires_at = now + 56 days` (8 weeks) so the hourly filler's candidate selector skips the row until the hand-edit expires. Hostnames are trimmed, deduped case-insensitively, and empties dropped before storage. (2) `DELETE /api/admin/hostname-lookup/entries/{ip}` (admin) drops a cache row entirely — 204 on success, 404 if no row exists; useful for clearing junk rows (e.g. a rapiddns row that picked up wrong hostnames) so the next filler pass re-queries. (3) `GET /api/hosts/{host_id}/hostnames` (authenticated user, not admin-only) returns the cache row for this host's IP as a `CacheEntryHostnamesResponse { ip, hostnames, source, queried_at, expires_at }`, or an empty-list response with `source=null` when the filler hasn't reached the host yet — the host detail page distinguishes "not enriched yet" from "no such host" (404). New service functions `update_cache_entry_manual(db, ip, hostnames)` and `delete_cache_entry(db, ip)` in `app/services/hostname_lookup.py` alongside a new `get_cache_row_for_ip(db, ip)` helper that returns the raw row (including stale entries) so the host detail panel can show "last cached 2 weeks ago" metadata on expired rows without the filter that `get_cached_hostnames` applies. New constants `MANUAL_EDIT_TTL_DAYS = 56` and `MANUAL_SOURCE_NAME = "manual"`. 13 new backend tests: PUT requires admin, PUT creates new row, PUT overwrites existing (hackertarget→manual flip verified), PUT empty list creates no_results row, PUT uses 8-week TTL (±1 day tolerance for clock boundary), PUT dedupes + strips whitespace + case-insensitive; DELETE requires admin, DELETE removes existing row, DELETE missing row returns 404; GET /hosts/:id/hostnames returns empty for uncached host, returns cached hostnames + metadata, unknown host returns 404, auth required. Frontend: (a) new hooks `useUpdateHostnameCacheEntry` + `useDeleteHostnameCacheEntry` in `features/hostname-lookup/hooks/useHostnameLookup.ts` invalidating the `["hostname-lookup"]` query key on success, plus `useHostCachedHostnames(hostId)` query hook for the host detail page. (b) Admin UI entries table gets a new "Actions" column with Pen (edit) + Trash (delete) icon buttons per row; the source badge for `manual` rows gets an amber `font-emphasis` accent so hand-edited rows stand out visually in long tables; delete click shows a `window.confirm` dialog with a "the next filler run will re-query this IP" hint before firing the mutation. (c) New modal `Dialog` component on the admin page opened by the Pen icon: `<Textarea>` prefilled with the current hostnames (one per line), live "N non-empty line(s)" counter, Save button that mutates via `useUpdateHostnameCacheEntry` and toasts a summary. Save with empty textarea explicitly marks the IP as `no_results` — useful for permanently silencing SNI fan-out on a specific host. (d) New `HostKnownHostnames` component on the host detail page between `HostComment` and the tabs, collapsible card showing the cached hostname count + source badge + "cached DATE" timestamp, click-to-expand shows all vhosts as chips. Component hides itself entirely when the backend returns `source=null` (no cache row yet — filler hasn't reached the host) so hosts without data don't get a noisy empty panel. Manual-source rows render with an amber "MANUAL" badge next to the source name. Explanatory blurb ("these are the vhosts nuclei will fan out over when SNI fan-out is enabled on this host's network") makes the panel's relevance obvious to users who don't know about SNI fan-out. Backend suite 352/352 green, frontend typecheck + lint + build + 218/218 vitest green. **Live-verified** against the dev backend: `PUT /entries/213.183.76.224` overwrote the real 35-vhost row from HackerTarget with two manual hostnames, stamped `source=manual` + `expires_at=2026-06-06` (exactly 56 days out); `GET /api/hosts/41/hostnames` read the same row back via the host detail path with matching metadata; `DELETE` returned 204 and dropped the row cleanly

- **RapidDNS fallback source for hostname lookup cache**: adds rapiddns.io as a second reverse-IP source after HackerTarget in the filler's source chain. New `RapidDnsSource` class in `app/services/hostname_lookup.py` targets `https://rapiddns.io/sameip/<ip>?full=1`, parses the HTML results table via a regex (`<td>([^<]{1,253})</td>`) + FQDN validation regex (RFC-ish: 1-253 chars, labels 1-63 chars, TLD ≥ 2 alpha), filters out IPv4 literals and non-FQDN noise like dashes/single-word cells, dedupes case-insensitively. Rate-limit handling matches HackerTarget: HTTP 429 or a Cloudflare challenge page (`cf-browser-verification` marker) or a captcha-without-table body maps to a `failed` result carrying the `"api count exceeded"` marker so `fill_cache_once` pins the budget and stops the source for the rest of the run. Plain browser User-Agent header set on the httpx client because some CDNs reject requests without one. Defensive body-too-small guard (< 100 chars → `no_results`) handles truncated / stub responses without crashing the parser. Two new settings in `core/config.py`: `rapiddns_enabled: bool = True` (toggle the whole source off without editing code) and `rapiddns_daily_limit: int = 100` (rapiddns doesn't publish a public rate limit — 100/day is a conservative default). Throttle interval reuses `RAPIDDNS_MIN_REQUEST_INTERVAL_S = 0.6` matching HackerTarget's 2 req/s ceiling. Filler integration: new `_build_source_chain()` helper in `hostname_lookup_filler.py` builds the ordered source list from settings (HackerTarget first, rapiddns second, disabled sources omitted), and `run_hostname_cache_filler` iterates the chain, calling `fill_cache_once` once per source inside one shared session with per-source try/except so a broken rapiddns never blocks HackerTarget from running tomorrow and vice versa. **The candidate-selector partitioning falls out for free**: `_select_candidate_ips` just asks "no fresh row or expired" without caring about source, so IPs HackerTarget already enriched in the same run are transparently skipped by rapiddns's pass — no schema change needed. Admin UI status endpoint (`get_cache_status`) surfaces rapiddns's budget row even before the first call so the dashboard always shows "100 remaining" from day 1. 10 new tests: 8 for `RapidDnsSource.parse` (table-extract happy path, empty/tiny body as no_results, Cloudflare challenge + captcha as failed-with-marker, table-with-no-FQDNs as no_results, fetch wraps parse, HTTP 429 rate-limit detection) + 2 for filler multi-source chaining (second source only touches IPs the first missed; second source runs when first's budget is pre-pinned). Backend suite 339/339 green. **Live-verified** on the dev stack: source chain `_build_source_chain()` correctly reports `[hackertarget(50, 0.6s), rapiddns(100, 0.6s)]`; admin status endpoint surfaces both budget rows; manual `/run-filler` trigger with HackerTarget already pinned at 50/50 (from earlier test runs) made 18 rapiddns calls in 20 s, cached 7 new success rows (jumped coverage from 9.2% to 15.6%), 11 no_results rows for IPs without shared-hosting neighbours on rapiddns — exactly the fallback pattern Commit 7 was meant to unlock

- **Admin UI `/admin/hostname-lookup`**: dedicated admin page for the hostname lookup cache at `/admin/hostname-lookup`, sidebar entry "Hostname Cache" with Globe icon under the admin group. Four overview stat cards at the top — "Cache coverage" (enriched / total hosts with %), "Total vhosts" (sum across all success rows, scoped as "across N success rows"), "HackerTarget budget" (used/limit today + remaining), "Filler schedule" (interval + last-queried-at timestamp or "disabled"). Entries table below with per-row IP, colored status badge (success emerald / no_results slate / failed rose), vhost count (right-aligned mono), source, queried timestamp, expires date. Rows with hostnames are click-expandable into an inline panel showing the full vhost list as chips and any failure-row error message in red. Filter bar with IP/hostname live search (client-side substring match, case-insensitive across ip + every hostname) and a status dropdown ("all / success only / no_results only / failed only"). Action buttons in the card header: "Export" (downloads the current cache as `opm-hostname-cache-YYYY-MM-DD.json` via Blob + object URL, disabled when cache is empty), "Import" (hidden file input, accepts JSON, validates `format_version === 1`, calls POST /import with the selected strategy, toasts a summary "X inserted, Y overwritten, Z skipped"), "Import strategy" dropdown ("skip existing" vs "overwrite"), "Run filler now" button in the page header (posts /run-filler and toasts "Filler job queued"). Page header has a "Refresh" button that re-fetches both status and cache. Status query auto-polls every 15 s via `refetchInterval` so the budget counter ticks in real time while a filler run is in progress. Two new backend endpoints back this: `GET /api/admin/hostname-lookup/status` (aggregates filler config, row counts by status, vhost total across all success rows, host-inventory coverage %, last_queried_at, per-source daily budgets with HackerTarget always surfaced even when no row exists yet) via `hostname_lookup_io.get_cache_status(db)`; `POST /api/admin/hostname-lookup/run-filler` (admin-only, returns 202 immediately, queues `run_hostname_cache_filler` via FastAPI `BackgroundTasks` so the filler runs out-of-band and the HTTP response doesn't block on the ~30-60 s throttled loop). New Pydantic schemas `CacheStatusByStatus`, `CacheBudgetStatus`, `CacheStatusResponse`, `CacheFillerRunResponse` in `app/schemas/hostname_lookup.py`. 8 new backend tests (empty-cache status, counts by status + vhosts, coverage %, expired-success-row not counted as enriched, status endpoint happy path, status endpoint admin gate, run-filler admin gate, run-filler returns 202) pushing backend total to 329/329 green. New frontend feature module `features/hostname-lookup/hooks/useHostnameLookup.ts` with 4 TanStack Query hooks (`useHostnameLookupStatus`, `useHostnameLookupExport`, `useRunHostnameCacheFiller`, `useImportHostnameCache`) and typed response shapes mirroring the backend Pydantic schemas. Frontend typecheck + lint + build + 218/218 vitest suite green. **Live-verified** against the dev backend: `GET /api/admin/hostname-lookup/status` returned `{total_entries: 17, total_vhosts: 249, total_hosts: 109, enriched_hosts: 10, coverage_percent: 9.2, budgets: [{source: "hackertarget", used: 50, limit: 50, remaining: 0}]}` — exactly the payload the admin page renders into the four stat cards

- **Nuclei SNI fan-out**: when a network has both `nuclei_enabled=true` and the new `nuclei_sni_enabled=true` (per-network toggle, default off, migration 015), the scanner now expands nuclei targets from `IP:PORT` to one target per cached vhost via the hostname lookup cache. Full pipeline end-to-end: (1) DB column `networks.nuclei_sni_enabled BOOLEAN NOT NULL DEFAULT 0` via Alembic 015, plumbed through Network model → NetworkCreate/Update/Response schemas → ScannerJobResponse → networks service with a safety guard that forces `nuclei_sni_enabled=false` whenever `nuclei_enabled` isn't true (no orphaned SNI flag). (2) New admin-less backend endpoint `GET /api/scanner/hostnames?ips=a,b,c` (scanner JWT required) backed by a new `get_hostnames_for_ips(db, ips)` service helper that bulk-reads fresh, non-failed rows from `hostname_lookup_cache`, omitting IPs without cached hostnames and rows with zero hostnames (so `no_results` entries don't show up). 500-IP request cap and whitespace/empty-token tolerance in the query-string parser. 6 new service tests + 6 new endpoint tests including admin-token-rejection to prove only scanner-scoped JWTs pass. (3) Scanner client gets `get_hostnames_for_ips(ips)` — best-effort with broad exception handling, returns `{}` on transport failure so a missing backend never fails the surrounding scan. New `nuclei_sni_enabled` field plumbed into `ScannerJob` dataclass and read from the `/api/scanner/jobs` payload. (4) `scanner/src/scanners/nuclei.py::build_targets()` signature extended with `known_hostnames: dict[str, list[str]] | None = None` and `max_vhosts_per_ip: int = 50`. When `known_hostnames` is provided, the function groups discovered ports by IP, substitutes cached vhosts for each IP's open web ports, picks scheme per-port via a well-known TLS port list (`443, 8443, 9443, 2083, 2087, 8089`) plus an nmap `service_guess` substring check for "ssl"/"https", and falls back to bare `IP:PORT` for IPs with no cached vhosts. 50-vhost-per-IP cap prevents a 500-vhost shared-hosting box from producing `500 × 3 ports × 6000 templates ≈ 9 M requests` per scan. 10 new nuclei build_targets tests: None-falls-back, empty-dict-falls-back, mixed IPs with/without hostnames, scheme selection per port, masscan no-service_guess TLS detection, multiplier (2 ports × 3 vhosts = 6 targets), vhost cap enforcement, non-web ports filtered regardless of hostnames, empty vhost list falls back to IP, deduplication across both IP and SNI targets. (5) `scanner/src/orchestration.py::_run_nuclei_phase` calls `client.get_hostnames_for_ips(unique_ips)` after port_scan when `job.nuclei_sni_enabled=true` and passes the result into `build_targets`. Failures are caught and fall back to IP-only targets with a warning log. Logs total vhost count so operators can see SNI fan-out size in the scan log UI. (6) Frontend NetworkForm's `NucleiSettings` component gets a second checkbox "SNI fan-out: scan every cached vhost on each IP" inside the existing collapse-when-nuclei-disabled section, with an explanatory subtitle noting the hostname-resolution caveat and the 50-vhost cap. `nuclei_sni_enabled` added to `NetworkFormData`, the NetworkForm `source` hydration, the payload builder, and the `Network` TypeScript type in `lib/types.ts`. Form submit force-clears the flag when `nuclei_enabled` is off or scanner_type is Greenbone, mirroring the backend guard. **Live-verified**: `GET /api/scanner/hostnames?ips=213.183.76.224,213.183.76.225` against the running dev backend returned the real cached vhost lists (35 + 6 hostnames across the two IPs) — exactly the data `build_targets` will expand into SNI fan-out targets on the next nuclei run. Full suites: backend 321/321 green, scanner 188/188 green, frontend typecheck + lint clean

- **Hostname lookup cache (export/import)**: two new admin-only endpoints back up and restore the hostname cache without touching the daily budget counter. `GET /api/admin/hostname-lookup/export` returns the full `hostname_lookup_cache` as a `CacheExportDocument` JSON (format_version, exported_at, source_instance, entry_count, entries[]) — rows sorted by IP for deterministic snapshot diffs, every status included (success/no_results/failed), per-row `queried_at` / `expires_at` / `error_message` preserved so an export → import(overwrite) round-trip is lossless. `POST /api/admin/hostname-lookup/import?strategy=skip|overwrite` accepts a `CacheImportRequest` with per-entry `HostnameLookupEntry` rows; `skip` preserves existing rows (safe bootstrap from a peer instance), `overwrite` replaces them (full restore from backup). Hand-written bootstrap documents can omit `queried_at` / `expires_at` — the service fills in `now` and `now + TTL(status)` via the standard `_ttl_for_status` (30d success / 7d no_results / 3d failed). Import accounting via `CacheImportSummary`: total, inserted, overwritten, skipped, rejected, errors[]. Format version locked at 1 with field_validator — unknown versions return 422. Strategy query param restricted by `pattern="^(skip|overwrite)$"` — unknown strategies return 422. Budget table (`hostname_lookup_budget`) is intentionally **untouched** by import so a bulk restore doesn't trick the filler into thinking it already burned today's API quota. 17 new unit + integration tests cover: export shape, status coverage, IP ordering, metadata preservation, import skip (insert + preserve existing), import overwrite (replaces), missing-timestamp backfill (30d TTL verified), export→import round-trip, HTTP admin gating (401/403 without token), endpoint response shapes, malformed format_version rejection, unknown-strategy rejection. **Live-verified** against the dev backend: curl `/api/admin/hostname-lookup/export` returned the real 17-entry cache snapshot in the expected shape

- **Hostname lookup cache (background filler job)**: new `backend/src/app/services/hostname_lookup_filler.py` wiring the service layer to APScheduler. `fill_cache_once(db, source, daily_limit, ...)` picks candidate IPs from the `hosts` table where either no cache row exists or `expires_at <= now`, ordered by `hosts.last_seen_at DESC` so freshly discovered hosts are enriched first. Control flow covers: early return on zero budget (`skipped_reason=budget_exhausted`), early return on empty candidate list (`skipped_reason=no_candidates`), per-IP commit inside the loop so a crash mid-run never loses already-fetched hostnames, 2-consecutive-non-429-failures abort (per-run only, next hour starts fresh — matches the "wenn crt.sh oder hackertarget uns nicht antwortet, frag in diesem scan nicht mehr dort, beim nächsten scan wieder" requirement), rate-limit pin on HTTP 429 or "API count exceeded" body, `asyncio.sleep(HACKERTARGET_MIN_REQUEST_INTERVAL_S=0.6)` between source calls to stay under HackerTarget's 2 req/s throttle, and `MAX_IPS_PER_RUN=25` hard cap so a 100/day API-key budget spreads evenly across ~24 hourly slots instead of burning the whole day at 00:00. Observability via `FillerRunSummary` dataclass (candidates, cache_hits, success, no_results, failed, rate_limited, budget_exhausted_mid_run, consecutive_failures_abort, processed_ips) logged structured at run completion. `run_hostname_cache_filler` is the APScheduler entry point — gated on `settings.hostname_lookup_enabled` (default `true`), opens a fresh session, selects the daily limit based on whether `settings.hackertarget_api_key` is set (50 anon, 100 with key). Wired into `start_scheduler()` as a second job alongside the existing `scan-schedule-evaluator`, with `IntervalTrigger(minutes=settings.hostname_lookup_interval_minutes=60)`, `max_instances=1`, `coalesce=True`, `misfire_grace_time=300`. Three new env var settings: `HOSTNAME_LOOKUP_ENABLED`, `HACKERTARGET_API_KEY`, `HOSTNAME_LOOKUP_INTERVAL_MINUTES`. 15 new unit tests cover candidate selection (empty DB, uncached hosts, fresh-cache skip, expired-cache pick, `last_seen_at DESC` ordering, limit respect), fill flow (no-candidates skip, budget-exhausted skip, successful multi-IP fill, `no_results` cache write, rate-limit pin mid-run, 2-consecutive-failure abort, failure-then-success counter reset, `max_ips` cap, budget-tighter-than-max cap). Full backend suite 292/292 green. **Verified end-to-end against the live dev DB**: manual invocation of `run_hostname_cache_filler` enriched 9 real production hosts in ~10s before hitting HackerTarget's rate limit; one IP (`213.183.76.224`) returned 35 actual vhosts (`bonifatiuskloster.de`, `derweinberg.de`, `jugendkloster.de`, ...) stored as a JSON array in `hostname_lookup_cache.hostnames_json` — exactly the SNI-fan-out data Commit 5 will feed into nuclei

- **Hostname lookup cache (service layer)**: new `backend/src/app/services/hostname_lookup.py` — pluggable reverse-IP hostname discovery with a `HostnameLookupSource` Protocol (so Commit 5 can add RapidDNS/Shodan paid/ViewDNS sources without touching cache code), `HackerTargetSource` implementation hitting the exact same `/reverseiplookup/` URL already used by `scanner/src/hostname_enrichment.py` but returning the full hostname list instead of `domains[0]`, and a high-level `lookup_with_cache(db, source, ip, daily_limit)` that glues cache-read / budget-consume / source-fetch / cache-write into one exception-free function. Per-outcome TTLs decided service-side: success 30 days (vhost lists change slowly), no_results 7 days (so dead IPs aren't re-hammered every filler pass), failed 3 days (transient outage retry). Budget counter (`hostname_lookup_budget` from migration 014) atomically incremented before each outbound call inside the caller's session — safe under APScheduler's single-instance filler-job assumption; docstring flags that parallel workers would need a row lock. HackerTarget free-tier limits: 50 req/day anonymous (not the 20 I initially guessed), throttled to 2 req/s max, returns HTTP 429 when either limit is exceeded; the source maps 429 to a `failed` result carrying an `"api count exceeded"` marker so `lookup_with_cache` can pin the daily counter to the cap and skip the source for the rest of the filler run, protecting against clock-skew or backend-restart wiping the counter mid-day. 30-case unit test suite covers parser classification (success/no_results/failed across empty body, real reverse-IP body, "No DNS" response, "API count exceeded" text body, HTTP 429, HTTP 500, generic connection error), cache semantics (miss vs no_results-hit vs failed-hit vs expired), budget (consume, exhaust, pin, per-source isolation), and the `lookup_with_cache` integration path (cache shortcut, cache miss writes row, budget exhaustion skips source, source-side rate-limit pins budget, no_results caches an empty list). Full suite 277/277 green

- **Hostname lookup cache (schema)**: new tables `hostname_lookup_cache` and `hostname_lookup_budget` added via migration 014, backing the upcoming SNI-aware reverse-IP hostname discovery pipeline. `hostname_lookup_cache` stores the full list of hostnames returned by an external reverse-IP source per IP (`hostnames_json`), not just the first one as today's `hostname_enrichment.py` does — the HackerTarget `reverseiplookup` endpoint returns 195 hostnames for a typical Plesk shared-hosting IP (verified empirically on `213.183.76.104` → 16 apex domains / 195 FQDNs), and the existing `enrich_hostnames_hackertarget` currently drops 194 of them via `results[ip] = domains[0]`. `hostname_lookup_budget` tracks per-source, per-UTC-day API call counts so an upcoming hourly filler job can honour free-tier rate limits (HackerTarget: 20/day without API key, 100/day with a free key). Schema intentionally keeps `no_results` and `failed` outcomes cacheable — status column distinguishes the three outcomes and `expires_at` lives per-row so the service layer can apply different TTLs per outcome (short for failed, medium for no_results, long for success) without burning budget on dead IPs at every filler run. Unique on `ip` (not `(ip, source)`) — simpler for a single-source start; noted in the model docstring that future multi-source merging will need a sub-table. Motivates nuclei SNI fan-out: a Plesk-style shared-hosting IP serves 100+ vhosts and we need the real hostname list per IP so nuclei can hit each vhost individually via SNI instead of only probing the default cert on `IP:443`

- **Per-network SSH probe toggle**: new `networks.ssh_probe_enabled` column (default `true`, migration 013) exposes a per-network opt-out for SSH probing. Prior to this change the scanner unconditionally ran `ssh-audit` and `nmap ssh-auth-methods` against any discovered SSH service after every successful port scan, with no way to suppress it — operators who did not want SSH fingerprinting on a given network (e.g. internal management ranges, noisy lab nets, or networks where SSH alerts are handled out-of-band) had to dismiss the findings after the fact. The new flag plumbs through Network model → NetworkCreate/Update/Response schemas → `ScannerJobResponse` → scanner `ScannerJob` model → `_submit_pipeline_results()` where it gates the existing SSH probe block in `scanner/src/orchestration.py`. When disabled, the scanner logs `"SSH probing disabled for this network, skipping"` and submits an empty `ssh_results` list with the port-scan payload; no rows are written to `ssh_scan_results` and no SSH alerts are generated for that scan. Frontend adds a `Run SSH probe on open ports` checkbox above the SSH Alert Overrides section in the Network form, defaulting on to preserve existing behavior for all legacy networks. Alerts configuration (global defaults + per-network overrides) is untouched and continues to apply whenever the probe is enabled

### Changed

- **Network form dialog**: split into three tabs — **General** (Network identity + Scanner configuration), **Phases** (Scan phases + NSE profile + Nuclei settings; hidden for Greenbone networks), **Schedule & Alerts** (cron + email + SSH probe + SSH overrides). Previously the form stacked all four fieldsets vertically, which forced significant scrolling even on wide screens once GVM, Nuclei, SSH probe, and SSH alert overrides had all been added. Tabs eliminate the scroll for the common edit path: most single-field changes now fit in one viewport. Dialog width reset from the experimental `lg:max-w-6xl` back to a tidier `sm:max-w-4xl` since tabs reduce the need for horizontal breathing room. All tab panels stay in the DOM via the `hidden` attribute (not conditional mount) so react-hook-form keeps every field registered across tab switches and submit-from-any-tab works. Validation errors on non-active tabs auto-switch the dialog to the tab holding the first bad field via a new `onInvalid` handler plus a field→tab map, so inline errors are always visible after a failed submit. Greenbone edge case handled via a derived `effectiveTab` (not a useEffect setState, to avoid cascading renders): when the user flips scanner_type to Greenbone while sitting on the Phases tab, the view falls back to General without touching tab state, and when they flip back it restores
...
- **Nuclei scanner**: nuclei phase was silently skipped on networks that had a stored `phases` column (the common case — every network persisted via the NetworkForm ends up with `[host_discovery, port_scan, vulnerability(nmap_nse, enabled=false)]`). The scanner's `process_job` used `job.phases or _build_legacy_phases(job)` and only `_build_legacy_phases` knew how to append a nuclei phase, so `nuclei_enabled=true` on the network was passed to the scanner but never acted on. Centralized nuclei injection in a new `_ensure_nuclei_phase(phases, job)` helper that runs for both legacy and pre-built phase lists; the helper is idempotent (no duplicate nuclei phase if one is already enabled) and returns a new list (immutability). Covered by 12 new unit tests in `scanner/tests/test_orchestration_phases.py`. Regression spotted on scan #40 (network "Copy of web-prod", scanner_type=nmap, nuclei_enabled=1) which completed with only the 2-phase pipeline `['host_discovery', 'port_scan']` and zero vulnerabilities stored
- **Scanner dev image**: `scanner/Dockerfile.dev` was missing the nuclei binary (only the production `scanner/Dockerfile` had the multi-stage `COPY --from=projectdiscovery/nuclei` step), so even if a nuclei phase had been appended correctly in `compose-dev.yml`, `_run_nuclei_phase` would have short-circuited with "nuclei: module import failed" or missing-binary logs. Mirrored the prod multi-stage copy + template-update RUN into `Dockerfile.dev`, pinned to the same `NUCLEI_VERSION=v3.7.1`
- **Nuclei templates**: the build-time `RUN nuclei -update-templates -ud ... -duc` step in both `scanner/Dockerfile` and the new `Dockerfile.dev` passed `-duc` (`-disable-update-check`), which *short-circuits the template update itself* — the directory ended up empty, and every nuclei scan then failed at runtime with `FTL: Could not run nuclei: no templates provided for scan` (13 170 templates expected, 0 found). Removed `-duc` from the update step. Runtime `-disable-update-check` is still passed from `run_nuclei` so a running container never drifts mid-scan. Verified via scan #41 on network 8 (`nmap` + `nuclei_enabled=1`): pipeline reports 3 phases, nuclei runs against 5 http/ssl targets
