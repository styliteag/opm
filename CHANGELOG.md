...
- **Hostname lookup queue** (migration 016): new `hostname_lookup_queue` table backs the on-demand manual lookup pipeline for the upcoming scanner-centric refactor (Plan C, 2.3.0). Operators trigger a reverse-IP lookup for a single host from the UI; the request is enqueued and the next scanner poll claims it, runs the HackerTarget / RapidDNS chain, posts results to `hostname_lookup_cache`, and marks the row completed. Lifecycle states: `pending` → `claimed` → `completed`/`failed`. Indexed on `(status, requested_at)` for the scanner poll and on `ip` for de-dup lookups. FK to `users.id` (`ON DELETE SET NULL`) preserves audit history when users are deleted. Multiple pending rows for the same IP allowed (de-duped client-side at claim time, since MariaDB < 10.5 can't enforce a partial unique index)
### Removed

- **Backend hostname cache filler**: the 2.2.0 background APScheduler filler that populated `hostname_lookup_cache` from the backend is gone. `backend/src/app/services/hostname_lookup_filler.py` + `backend/tests/test_hostname_lookup_filler.py` deleted entirely. The scheduler no longer registers the `hostname-cache-filler` job. The scanner is the only egress point for external hostname APIs in 2.3.0 — manual refreshes flow via `hostname_lookup_queue` + `process_hostname_lookup_queue`, and discovery-phase enrichment posts results via `POST /api/scanner/hostname-results`.
- **Backend source classes + `lookup_with_cache`**: `HackerTargetSource`, `RapidDnsSource`, the `HostnameLookupSource` Protocol, `lookup_with_cache`, `_is_probably_ipv4`, and all their supporting constants (`HACKERTARGET_URL/TIMEOUT/MIN_REQUEST_INTERVAL_S`, `RAPIDDNS_URL_TEMPLATE/TIMEOUT/USER_AGENT/DEFAULT_DAILY_LIMIT/MIN_REQUEST_INTERVAL_S`, `_RAPIDDNS_TD_RE`, `_FQDN_RE`) removed from `app.services.hostname_lookup`. `httpx` and `re` imports dropped from the module — the backend no longer makes outbound hostname API calls. Same classes still live scanner-side in `scanner/src/hostname_sources.py`. Tests covering parse/fetch semantics moved to `scanner/tests/test_hostname_sources.py`.
- **`POST /api/admin/hostname-lookup/run-filler` endpoint**: returns 404. `CacheFillerRunResponse` schema deleted. Frontend `useRunHostnameCacheFiller` hook deleted in Commit 9; this commit removes the server-side counterpart.
- **`filler_enabled` + `filler_interval_minutes` from `CacheStatusResponse`**: backend schema + service builder no longer return these fields; frontend type no longer declares them. The admin page stopped reading them in Commit 9 (replaced with the `pending_queue_count` card).
- **`hostname_lookup_enabled` + `hostname_lookup_interval_minutes` settings**: removed from `Settings` in `app.core.config`. The remaining `hackertarget_api_key`, `rapiddns_enabled`, `rapiddns_daily_limit` settings stay — they still feed the scanner-facing `GET /api/scanner/hostname-budget` endpoint and the admin `/status` dashboard's per-source limit labels.
- **Backend tests**: dropped `TestHackerTargetParse`, `TestHackerTargetFetch`, `TestLookupWithCache`, `TestRapidDnsParse` classes from `test_hostname_lookup_service.py` (30 tests total); dropped `TestRunFillerEndpoint` from `test_hostname_lookup_io.py`. Added a pin test asserting `filler_enabled` / `filler_interval_minutes` are absent from the `/status` payload so nothing accidentally re-adds them. Full backend suite: **359 passing** (down from 405 = 46 filler-era tests removed). Live-verified end-to-end after backend restart: clean startup, `/status` returns the new payload with `pending_queue_count`, `/run-filler` returns 404, admin page renders without filler references.

### Added

- **Admin `/admin/hostname-lookup` page simplification**: reworked for the scanner-centric architecture. Header: "Run filler now" button removed (scanner owns enrichment cadence now); the hint text now describes the scanner-driven path and the per-row Refresh affordance. Status cards: "Filler schedule" replaced with "Pending lookups" showing `pending_queue_count` (rows in `pending` or `claimed` state) with a "queue empty — scanner is idle" / "awaiting scanner poll" secondary line. Entries table: per-row RefreshCw icon button between Pencil and Trash fires `useRefreshCacheEntry(ip)` to enqueue a manual re-lookup for an arbitrary IP via `POST /api/admin/hostname-lookup/entries/{ip}/refresh`; toasts "Hostname lookup queued for {ip} — results appear on next scanner poll (~10 s)" and schedules a 10-second delayed invalidation of the `hostname-lookup` query key so the status cards + entries table refetch after the scanner drains the queue. The `useRunHostnameCacheFiller` hook is removed (no callers remain). Backend: extended `CacheStatusResponse` with `pending_queue_count: int = 0` populated from `get_pending_queue_count(db)`. **Live-verified end-to-end** in Chrome: the page rendered with the new stat card, Pencil/Refresh/Trash action order confirmed on a real row, clicking Refresh on 10.20.3.254 enqueued job 5, scanner processed it within 1 s (HT exhausted → RapidDNS → completed), and the Pending lookups card stayed at 0 because the refetch caught the scanner after completion
- **Host detail panel Refresh button**: `HostKnownHostnames` on the `/hosts/{id}` detail page gains an always-visible Refresh button next to the source / queried-at meta. Clicking it fires the new `useRefreshHostHostnames(hostId)` mutation — POSTs to `/api/hosts/{hostId}/hostname-lookup/refresh`, toasts "Hostname lookup queued — results appear on next scanner poll (~10 s)", and schedules a 10-second delayed invalidation of the `["host-hostnames", hostId]` + `["hosts"]` React Query keys so the panel auto-refreshes once the scanner has drained the queue. The panel now always renders: when no cache row exists yet, a hint explains "No cached hostnames yet. Click Refresh to enqueue a reverse-IP lookup" so operators can trigger first-time enrichment directly from the host detail page. Clicking the refresh button stops propagation so the header expand-toggle doesn't fire. **Live-verified end-to-end** in Chrome: clicked Refresh on host 213.183.76.104 (no cache row) → toast appeared → scanner picked up job 4 within 1 s → HT budget exhausted → RapidDNS resolved 100 hostnames → panel auto-refreshed to `Known Hostnames (100)` after the 10-s delay → expanding the panel rendered all 100 vhost chips
- **Hosts table cached vhost chip**: `/hosts` table now surfaces the hostname lookup cache inline. The hostname column falls back to `cached_display_hostname` when the user-set `host.hostname` is null, and a compact `+N` chip appears next to rows with more than one cached vhost. Hover opens a Radix tooltip with the full vhost list (fetched via `useHostCachedHostnames(hostId)` only after the tooltip first opens — `hasOpened` flag keeps subsequent re-opens instant from the React Query cache). Backend support: extended `HostResponse` with two new optional fields (`cached_hostname_count: int = 0`, `cached_display_hostname: str | None = None`), populated in `GET /api/hosts` via a single batch JOIN against `hostname_lookup_cache` (new `get_cached_hostname_summaries_for_ips` service + `CachedHostnameSummary` dataclass). `no_results` rows surface as `count=0` (distinct from missing-row 0 on the backend but collapsed to the same UI chip-less render); `failed` / expired rows are treated as cache miss. User-set hostnames always win so inline-edit precedence is unchanged. 4 new backend tests pin the JOIN contract. **Live-verified end-to-end** in Chrome against the dev stack: 8 chips rendered across the first 50-row page, hovering a `+1` chip loaded the two-vhost list with `source: hackertarget` footer, nulling a host's `hostname` via PATCH caused the row to fall through to the cached `solo.example`
- **Scanner hostname-lookup queue poller**: closes the loop on the on-demand "Refresh" UX. New `process_hostname_lookup_queue(client, logger, limit=10)` in `scanner/src/hostname_enrichment.py` is wired into `scanner/src/main.py`'s poll loop alongside scan-job and host-discovery polling. Each cycle: claims up to N pending rows via `GET /api/scanner/hostname-lookup-jobs` (atomic pending → claimed on the backend), pre-flights `GET /api/scanner/hostname-budget` once for the whole batch, and runs the HackerTarget → RapidDNS chain per IP. Results post via `POST /api/scanner/hostname-results` (cache + budget counters update); each job is closed via `POST /api/scanner/hostname-lookup-jobs/{id}/complete` with `completed` (any source returned success) or `failed` (most-informative error from the source chain, or `"no results from any source"` when both came back empty). HT rate-limit signal pins the local budget map so subsequent jobs in the same batch skip HT honestly without burning more quota. Per-job exceptions are fire-and-forget — logged and the job is marked `failed` with the exception text. Skipped on GVM-only containers (they're not the egress point for hostname enrichment). Two new `ScannerClient` methods: `get_hostname_lookup_jobs(limit)` (returns `list[HostnameLookupJob]`, best-effort) and `complete_hostname_lookup_job(id, status, error?)` (best-effort POST, error truncated to 500 chars). New `HostnameLookupJob` dataclass in `scanner/src/models.py`. 28 new tests cover the poller (empty queue, transport error, HT happy path, RapidDNS fallback, both fail, rate-limit pinning across batch, pre-flight zero-budget skip, per-job exception → failed) and the two client methods (decode, empty/non-200/transport-error → empty, error truncation). **Live-verified end-to-end**: enqueued `1.1.1.1` via the admin API, scanner picked it up within 5 s, HT was at 0 budget so it went straight to RapidDNS, returned 100 hostnames, cache row written, queue row marked completed
- **Scanner enrichment refactor — multi-value + budget-aware + cache-driving**: `scanner/src/hostname_enrichment.py` now drives the backend cache. All five existing enrichment functions return `dict[str, list[str]]` (uniform shape — ssl-cert / Google DNS / ip-api / crt.sh return single-element lists, HackerTarget returns the **full** vhost list it was previously dropping with `[0]`). New `enrich_hostnames_rapiddns` slot uses the `RapidDnsSource` class as the HackerTarget fallback. HT and RapidDNS now delegate per-IP fetching to the source classes from Commit 4 instead of inline httpx loops, with the same rate-limit politeness sleep. New `post_hostname_results_to_backend(client, results, logger)` helper filters to vhost-list sources (HT / RapidDNS / crt.sh) before posting — display-name sources never reach the cache. `enrich_host_results(hosts, logger, client=None)` now optionally pre-flights `GET /api/scanner/hostname-budget` and skips HT/RapidDNS when `remaining=0`, then posts vhost-list outcomes to `POST /api/scanner/hostname-results` so the backend cache + per-source budget counters update. Legacy `client=None` path preserved for tests and any caller without a `ScannerClient` handy. `run_host_discovery` and `process_host_discovery_job` thread the client through. Two new `ScannerClient` methods: `get_hostname_budget()` (returns `{source: remaining}`, best-effort) and `post_hostname_results(results)` (best-effort POST with logged outcome). 20 new tests cover budget pre-flight skipping, post-back filtering, multi-value HT extraction, RapidDNS new function, and the two client methods. Live-verified inside the scanner container against MariaDB
- **Scanner-side reverse-IP hostname sources**: new module `scanner/src/hostname_sources.py` ports `HackerTargetSource` + `RapidDnsSource` from the backend's `app.services.hostname_lookup` to the scanner. Synchronous (`httpx.Client`) instead of async (`httpx.AsyncClient`) to match scanner conventions; otherwise the parse semantics are byte-identical so cache rows look the same regardless of which side of the refactor wrote them. Includes the `HostnameLookupResult` dataclass, `HostnameLookupSource` Protocol, and the per-source rate-limit constants. The classes are intentionally DB-unaware: budget enforcement and cache writes go through the backend via the `/api/scanner/hostname-budget` + `/api/scanner/hostname-results` endpoints added in the previous commit. Both copies coexist during the 2.3.0 transition; Commit 10 deletes the backend originals once the scanner-driven path proves out. 22 new tests in `scanner/tests/test_hostname_sources.py` mirror the backend's parse + fetch contract (deduplication, IPv4 literal stripping, error capping, HTTP 429 → "api count exceeded" marker, Cloudflare challenge detection)
- **Scanner-facing hostname results + budget API**: two new endpoints close the loop in the scanner-centric refactor. `GET /api/scanner/hostname-budget` (scanner JWT) returns today's per-source `{used, limit, remaining}` for HackerTarget and RapidDNS so the scanner can pre-flight before burning quota — limits resolve from `app.core.config.settings` (HT API key → 100/day, RapidDNS toggle → configured cap), `remaining` clamps to zero. `POST /api/scanner/hostname-results` (scanner JWT) accepts a bulk batch of enrichment outcomes with per-result `{ip, source, status, hostnames, error_message}`; for each entry the backend post-fact-increments the budget counter, upserts the cache row via the existing TTL pipeline (30d/7d/3d by status), backfills `host.hostname` to the first vhost iff the host exists and currently has no hostname (preserves manual edits + ssl-cert/PTR/ip-api enrichment), and pins the source's daily budget when an `"api count exceeded"` marker appears in the error message. Pydantic Literal validation rejects unknown sources with a 422 before they reach the service. Service primitives `get_scanner_budget_snapshot` + `apply_scanner_hostname_results` + `increment_budget_used` live in `app.services.hostname_lookup`. 21 new tests cover the service layer (snapshot, budget zeroing, host backfill semantics, no-overwrite, rate-limit pin) and HTTP endpoints (auth gate, normalisation, multi-source batch, 422 on bad source, post-pin GET budget). Live-verified end-to-end on the dev stack with mixed-status batches against MariaDB
- **Hostname lookup queue API**: backend service + endpoints to drive the new `hostname_lookup_queue` table. Service primitives (`enqueue_hostname_lookup`, `claim_pending_lookup_jobs`, `mark_queue_entry_completed`, `get_pending_queue_count`) live in `app.services.hostname_lookup` alongside the existing cache helpers; the claim path is single-statement atomic, dedupes same-IP rows within a batch, lazy-resweeps stuck `claimed` rows older than 1h, and lazy-GCs terminal rows older than 7d so the table stays bounded without a scheduled job. New endpoints: `POST /api/hosts/{host_id}/hostname-lookup/refresh` (any authenticated user can refresh a host's IP), `POST /api/admin/hostname-lookup/entries/{ip}/refresh` (admin-only, enqueues for an arbitrary IP), `GET /api/scanner/hostname-lookup-jobs?limit=N` (scanner JWT, atomically claims up to N pending rows), `POST /api/scanner/hostname-lookup-jobs/{id}/complete` (scanner reports terminal state, error bounded to 500 chars). 28 new tests cover the service layer + endpoints. Live-verified end-to-end on the dev stack: enqueue → claim → complete/failed → empty queue
- **Hostname lookup cache** (migrations 014, 015): reverse-IP vhost discovery pipeline for shared-hosting targets. New tables `hostname_lookup_cache` (IP → vhost list, per-row TTL by status: 30d success / 7d no_results / 3d failed) and `hostname_lookup_budget` (per-source/day API call counter). Pluggable `HostnameLookupSource` Protocol with two sources: `HackerTargetSource` (50/day anon or 100/day with `HACKERTARGET_API_KEY`, HTTP 429 → pin budget) and `RapidDnsSource` (rapiddns.io `/sameip/` HTML fallback, 100/day default, Cloudflare-challenge → pin budget). Background APScheduler filler runs hourly (`HOSTNAME_LOOKUP_INTERVAL_MINUTES=60`), picks `last_seen_at DESC` candidates, throttles 2 req/s, commits per-IP so crashes don't lose progress, bails on 2 consecutive non-429 failures, chains sources so rapiddns transparently fills gaps HackerTarget missed
- **Nuclei SNI fan-out**: per-network `nuclei_sni_enabled` flag expands nuclei targets from `IP:PORT` to one `https://vhost:PORT` per cached vhost using the hostname lookup cache. Scanner calls `GET /api/scanner/hostnames` between port_scan and nuclei phases. `build_targets()` picks scheme per-port via TLS port list + nmap `service_guess`, caps at 50 vhosts/IP to keep worst-case bounded, falls back to `IP:PORT` for IPs without cached hostnames. NetworkForm gets a second checkbox under the Nuclei section
- **Admin UI `/admin/hostname-lookup`**: new sidebar entry "Hostname Cache". Four stat cards (coverage %, total vhosts, per-source daily budgets, filler schedule, auto-refetched every 15s), filterable entries table with status badges, click-to-expand vhost chips, Pen/Trash action buttons per row. Export downloads full cache as JSON; Import accepts the same format with `skip`/`overwrite` strategy. "Run filler now" button queues an immediate filler pass via `BackgroundTasks`. Manual rows (see next bullet) get an amber accent
- **Hostname cache manual edit**: operators can hand-curate cached vhost lists. `PUT /api/admin/hostname-lookup/entries/{ip}` replaces a row with `source='manual'` and an 8-week TTL so the filler skips it until it expires (56-day `MANUAL_EDIT_TTL_DAYS`). Empty list stores a `no_results` marker (explicit "nothing to scan"). `DELETE /api/admin/hostname-lookup/entries/{ip}` drops a row so the filler re-queries. New modal dialog on the admin page with a textarea (one hostname per line) + live counter. New `HostKnownHostnames` collapsible card on the host detail page between HostComment and tabs, shows count + source + "cached DATE" with a chip grid on expand, amber "MANUAL" badge for hand-edited rows, hidden entirely when no cache row exists (`source=null`)
- **Per-network SSH probe toggle** (migration 013): `networks.ssh_probe_enabled` (default on) exposes a per-network opt-out so internal management ranges or noisy lab nets don't get automatic `ssh-audit` + `nmap ssh-auth-methods` after every port scan. Frontend checkbox in the Schedule & Alerts tab. Global SSH alert defaults are untouched

### Changed

- **Network form dialog**: split into three tabs — General / Phases / Schedule & Alerts — to eliminate the vertical scroll after GVM, Nuclei, SSH probe, and SSH alert overrides had all landed in one form. Validation errors on inactive tabs auto-focus their tab via a field→tab map. Greenbone edge case handled via a derived `effectiveTab` so flipping scanner_type to Greenbone while on the Phases tab falls back to General without touching tab state
- **Nuclei scanner**: `-stats -stats-interval 15` progress now streams live to scan logs instead of being swallowed by `subprocess.run(capture_output=True)`. Switched to `Popen(stdout=PIPE, stderr=STDOUT, bufsize=1)` with the existing `ProcessTimeoutWatcher` pattern; each `[INF] | Templates | Hosts | RPS | Requests` line is forwarded via `logger.info` so operators see nuclei progress in the UI
- **Network form**: edit-mode submit button now reads "Save" instead of "Update"

### Fixed

- **Nuclei scanner**: nuclei phase was silently skipped on networks with a stored `phases` column (the common case — every network persisted via NetworkForm has a pre-built phase list). `process_job` used `job.phases or _build_legacy_phases(job)` and only `_build_legacy_phases` knew how to append a nuclei phase, so `nuclei_enabled=true` was ignored for any non-legacy network. New `_ensure_nuclei_phase(phases, job)` helper runs for both legacy and pre-built phase lists; idempotent, returns a new list
- **Scanner dev image**: `scanner/Dockerfile.dev` was missing the nuclei binary (only `scanner/Dockerfile` had the multi-stage copy), so `_run_nuclei_phase` short-circuited with "nuclei: module import failed" in dev. Mirrored the prod multi-stage copy + template-update RUN into `Dockerfile.dev`, pinned to `NUCLEI_VERSION=v3.7.1`
- **Nuclei templates**: the build-time `nuclei -update-templates ... -duc` step passed `-duc` (`-disable-update-check`) which short-circuits the template update itself. Directory ended up empty and every run failed with "no templates provided for scan". Removed `-duc` from the update step; runtime `-disable-update-check` in `run_nuclei` still prevents mid-scan drift
- **Network form**: leaving Nuclei "Minimum severity" at the default `""` option silently failed client-side validation (zod enum rejected empty string) and Save did nothing. Preprocess empty string → `undefined` so the default option round-trips as "use backend default"
